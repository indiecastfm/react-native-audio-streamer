 ...
 */
public class Playback extends ReactContextBaseJavaModule implements AudioManager.OnAudioFocusChangeListener,
        OnCompletionListener, OnErrorListener, OnPreparedListener, OnSeekCompleteListener {

    private static final String TAG = "AudioStreamer/Playback";
    ...
    public Playback(ReactApplicationContext context /*MusicService service, MusicProvider musicProvider*/) {
    ...
    @ReactMethod public void stop(boolean notifyListeners) {
    ...
    @ReactMethod public void play() {
        ...
        boolean mediaHasChanged = !TextUtils.equals(mUrlString, mCurrentMediaId);
        if (mediaHasChanged) {
            mCurrentPosition = 0;
            mCurrentMediaId = mUrlString;
        }
    ...
    @ReactMethod public void seekToTime(int position) {
    ...
    @ReactMethod public void status(com.facebook.react.bridge.Callback callback) {
        callback.invoke(null, status);
    }

    @ReactMethod public void currentTime(com.facebook.react.bridge.Callback callback) {
        if (mMediaPlayer == null){
            callback.invoke(null,(double)0);
        }else{
            callback.invoke(null,(double)(mMediaPlayer.getCurrentPosition()/1000));
        }
    }

    @ReactMethod public void duration(com.facebook.react.bridge.Callback callback) {
        if (mMediaPlayer == null){
            callback.invoke(null,(double)0);
        }else{
            callback.invoke(null,(double)(mMediaPlayer.getDuration()/1000));
        }
    }
    ...
    private void createMediaPlayerIfNeeded() {
        ...
        if (mMediaPlayer == null) {
            ...
            mMediaPlayer.setWakeMode(context.getApplicationContext(),
